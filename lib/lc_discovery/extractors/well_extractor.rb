...
class WellExtractor
  ...
  def extract
    ...
    begin
      ...
      proj_id_germ = "#{@project} #{@label}" #ensure this matches clowder
      ...
      doc = {
        ...
        label: @label,
        project: @project,
        project_id: Utility.lc_id(proj_id_germ),
  ...
  def thing
    ...
    sql = 'select * from WellHeader'
    #@gxdb[sql].all.each do |x|
    @gxdb[:wellheader].order('Well ID'.to_sym).paged_each(rows_per_fetch: 500) do |x|
      #puts x['well id'.to_sym]
